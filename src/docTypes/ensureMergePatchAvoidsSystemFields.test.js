...
test('Reject patches that reference system fields.', () => {
  ...
  expect(() => ensureMergePatchAvoidsSystemFields({ docType: 'exampleDocType' })).toThrow(SengiInvalidMergePatchError)
  expect(() => ensureMergePatchAvoidsSystemFields({ docHeader: {} })).toThrow(SengiInvalidMergePatchError)
})
